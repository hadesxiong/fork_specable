...
class VersionHistoryDB {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORE_NAME)) {
          ...
          store.createIndex('fileId_timestamp', ['fileId', 'timestamp'], {
            unique: false,
          })
        }
      }
  ...
  async saveSnapshot(
    ...
    await this.withTransaction<IDBValidKey>('readwrite', (store) =>
      store.add(snapshot),
    )
    return snapshot
  }
  ...
  async deleteSnapshot(id: string): Promise<void> {
    await this.withTransaction<undefined>('readwrite', (store) =>
      store.delete(id),
    )
  }

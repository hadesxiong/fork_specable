...
const breakingChangeRules: BreakingChangeRule[] = [
  ...
  {
    pattern:
      /^paths\.([^.]+)\.(get|post|put|patch|delete|options|head|trace)\.parameters\.(\d+)$/,
    check: (change, _oldSpec, newSpec) => {
      if (change.kind === 'N' && change.path) {
        ...
        if (param && !('$ref' in param) && param.required) {
          return {
            breaking: true,
            reason: `Added required parameter: ${param.name}`,
          }
        }
      }
      return { breaking: false }
  ...
  {
    pattern:
      /^paths\.([^.]+)\.(get|post|put|patch|delete|options|head|trace)\.responses\.(2\d{2}|default)$/,
    check: (change) => {
      if (change.kind === 'D') {
  ...
  {
    pattern:
      /^paths\.([^.]+)\.(get|post|put|patch|delete|options|head|trace)\.parameters\.(\d+)\.schema\.type$/,
    check: (change) => {
      if (change.kind === 'E') {
        return {
          breaking: true,
          reason: `Changed parameter type from ${change.lhs} to ${change.rhs}`,
        }
      }
      return { breaking: false }
    },
  ...
  },
  {
    ...
    check: (change) => {
      if (change.kind === 'A' && change.item?.kind === 'N') {
        return {
          breaking: true,
          reason: `Made property required: ${change.item.rhs}`,
        }
      }
      return { breaking: false }
    },
...
export function generateChangelog(result: DiffResult): string {
  ...
  lines.push('---')
  lines.push(
    `*Summary: ${result.summary.added} added, ${result.summary.removed} removed, ${result.summary.modified} modified (${result.summary.breaking} breaking)*`,
  )

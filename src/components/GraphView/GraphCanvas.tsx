...
import type { GraphData, GraphNode, GraphEdge, GraphEdgeType } from '../../workers/types';
...
function calculateNodeSize(node: GraphNode): { width: number; height: number } {
  ...
  const height = HEADER_HEIGHT + (displayCount + (hasMore ? 1 : 0)) * ROW_HEIGHT + BOX_PADDING;

  return { width, height };
}

function layoutNodes(nodes: GraphNode[], edges: GraphEdge[]): Map<string, NodePosition> {
  ...
  if (endpointNodes.length > 0) {
    ...
    for (const node of endpointNodes) {
      const { width, height } = calculateNodeSize(node);
      positions.set(node.id, { x: -width - GRID_GAP_X, y: endpointY, width, height });
...
export function GraphCanvas({ data, onNodeClick }: GraphCanvasProps) {
  ...
  const drawEdges = useCallback(
    (edges: GraphEdge[], positions: Map<string, NodePosition>, highlighted: string | null) => {
      ...
      for (const edge of edges) {
        ...
        const isHighlighted =
          highlighted !== null && (edge.source === highlighted || edge.target === highlighted);
        ...
        if (edge.sourceProperty) {
          ...
          if (node?.properties) {
            const propIndex = node.properties.findIndex((p) => p.name === edge.sourceProperty);
            if (propIndex !== -1 && propIndex < MAX_PROPERTIES) {
              startY = sourcePos.y + HEADER_HEIGHT + propIndex * ROW_HEIGHT + ROW_HEIGHT / 2;
            }
          }
        }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      viewport.drag().pinch().wheel().decelerate().clampZoom({ minScale: 0.1, maxScale: 2 });
      ...
      for (const node of data.nodes) {
        ...
        displayProps.forEach((prop, index) => {
          ...
          if (prop.refTarget) {
            ...
            indicator.position.set(pos.width - BOX_PADDING - (prop.required ? 20 : 8), y + 5);

...
describe('refPathToSourceMapKey', () => {
  it('converts simple component reference', () => {
    expect(refPathToSourceMapKey('#/components/schemas/Pet')).toBe('components.schemas.Pet');
  });

  it('converts nested reference', () => {
    expect(refPathToSourceMapKey('#/components/schemas/Order/properties/id')).toBe(
      'components.schemas.Order.properties.id'
    );
  });

  it('converts path reference', () => {
    expect(refPathToSourceMapKey('#/paths/~1users/get')).toBe('paths.~1users.get');
  });
...
describe('$ref pattern matching', () => {
  ...
  it('finds multiple refs in a line', () => {
    const text = 'items: { $ref: "#/components/schemas/Pet" } other: { $ref: "#/components/schemas/Order" }';
    ...
    expect(matches).toEqual(['#/components/schemas/Pet', '#/components/schemas/Order']);
  });

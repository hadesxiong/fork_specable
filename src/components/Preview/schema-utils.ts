...
export function schemaToTypeScript(
  ...
  function convertType(s: SchemaObject, indent = 0): string {
    ...
    // Handle composition
    if (schemaObj.allOf) {
      const types = schemaObj.allOf.map((sub) => convertType(sub as SchemaObject, indent));
      return types.join(' & ');
    }
    if (schemaObj.oneOf) {
      const types = schemaObj.oneOf.map((sub) => convertType(sub as SchemaObject, indent));
      return types.join(' | ');
    }
    if (schemaObj.anyOf) {
      const types = schemaObj.anyOf.map((sub) => convertType(sub as SchemaObject, indent));
      return types.join(' | ');
    }
        ...
        if (schemaObj.additionalProperties) {
          const valueType =
            typeof schemaObj.additionalProperties === 'object'
              ? convertType(schemaObj.additionalProperties as SchemaObject, indent)
              : 'unknown';
  ...
  function wrapNullable(typeStr: string, schemaObj: OpenAPIV3.SchemaObject): string {
  ...
  const resolvedSchema = isRef(schema) ? resolveRef(schema, spec)?.schema : schema;
  if (!resolvedSchema) {

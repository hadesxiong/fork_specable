...
describe('getSchemaType', () => {
  ...
  it('includes format in type string', () => {
    const spec = createSpec()
    expect(getSchemaType({ type: 'string', format: 'uuid' }, spec)).toBe(
      'string (uuid)',
    )
    expect(getSchemaType({ type: 'integer', format: 'int64' }, spec)).toBe(
      'integer (int64)',
    )
  })
  ...
  it('handles array types', () => {
    const spec = createSpec()
    expect(
      getSchemaType({ type: 'array', items: { type: 'string' } }, spec),
    ).toBe('array<string>')
  })

  it('handles $ref by returning the schema name', () => {
    const spec = createSpec({ User: { type: 'object' } })
    expect(getSchemaType({ $ref: '#/components/schemas/User' }, spec)).toBe(
      'User',
    )
  })
...
describe('resolveRef', () => {
  it('returns null for non-existent refs', () => {
    const spec = createSpec()
    expect(
      resolveRef({ $ref: '#/components/schemas/Missing' }, spec),
    ).toBeNull()
  })
...
describe('collectRefs', () => {
  it('collects refs from properties', () => {
    ...
    expect(refs).toEqual(
      new Set(['#/components/schemas/User', '#/components/schemas/Address']),
    )
  })
  ...
  it('collects refs from oneOf variants', () => {
    ...
    const refs = collectRefs(schema)
    expect(refs).toEqual(
      new Set(['#/components/schemas/Cat', '#/components/schemas/Dog']),
    )
  })

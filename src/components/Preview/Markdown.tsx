...
const components: Components = {
  ...
  code: ({ className, children, ...props }) => {
    ...
    return (
      <code className="px-1 py-0.5 bg-zinc-800 rounded text-zinc-300 text-[0.9em]" {...props}>
        {children}
      </code>
    );
  },
  ...
  th: ({ children }) => (
    <th className="px-2 py-1 text-left font-medium text-zinc-300 border border-zinc-700">{children}</th>
  ),
  td: ({ children }) => (
    <td className="px-2 py-1 text-zinc-400 border border-zinc-700">{children}</td>
  ),
  hr: () => (
